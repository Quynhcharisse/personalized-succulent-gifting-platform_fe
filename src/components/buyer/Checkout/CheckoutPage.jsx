...
import { removeItem, incrementQuantityBySize } from "../../../store/slices/cartSlice.js";
...
export default function CheckoutPage() {
  ...
  return (
    <>
        ...
        <Card sx={{ ...DASHBOARD_STYLES.paper }}>
          <CardContent sx={{ p: { xs: 2, sm: 3 } }}>
            ...
            <Box sx={{ p: { xs: 1.5, sm: 2 }, backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: 2, border: `1px solid ${COLORS.primary}20`, mb: 3 }}>
              <Box sx={{ display: 'flex', alignItems: { xs: 'flex-start', sm: 'center' }, justifyContent: 'space-between', flexDirection: { xs: 'column', sm: 'row' }, gap: { xs: 1, sm: 0 } }}>
                <Box sx={{ flex: 1 }}>
                  {loadingAddress ? (
                    <Typography variant="subtitle2" fontWeight={600} sx={{ color: COLORS.primary }}>Đang tải địa chỉ...</Typography>
                  ) : (
                    <>
                      <Typography variant="subtitle2" fontWeight={600} sx={{ color: COLORS.primary }}>{selectedAddress?.shippingAddress || "Chưa có địa chỉ"}</Typography>
                      <Typography variant="body2" sx={{ mt: 0.5, color: COLORS.primaryLight }}>{selectedAddress?.address || ""}</Typography>
                    </>
                  )}
                </Box>
                <Button size="small" sx={{ textTransform: 'none', color: 'white', fontWeight: 600, ...DASHBOARD_STYLES.primaryButton, alignSelf: { xs: 'flex-end', sm: 'auto' } }} onClick={() => setShowDialog(true)}>THAY ĐỔI</Button>
              </Box>
            </Box>
            ...
            {/* Table header */}
            <Box sx={{ display: { xs: "none", md: "grid" }, gridTemplateColumns: "1fr 140px 160px 1fr 120px", gap: 2, px: 2, py: 1.5, backgroundColor: COLORS.primary, borderRadius: 2, color: 'white', fontWeight: 600, marginTop: 3 }}>
            ...
            <Stack direction="column" spacing={1}>
              {items.map((item) => (
                <Box key={`${item.id}-${item.size}`} sx={{ display: { xs: "block", sm: "block", md: "grid" }, gridTemplateColumns: "1fr 140px 160px 1fr 120px", gap: 2, alignItems: "center", px: { xs: 1.5, sm: 2 }, py: 1.5, backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: 2, border: `1px solid ${COLORS.primary}20`, '&:hover': { backgroundColor: 'rgba(11,63,49,0.05)', transform: 'translateY(-1px)', boxShadow: '0 4px 12px rgba(11,63,49,0.1)' }, transition: 'all 0.2s ease' }}>
                  {/* Mobile */}
                  <Box sx={{ display: { xs: 'block', sm: 'block', md: 'none' } }}>
                    <Stack direction="row" alignItems="center" spacing={1.5} sx={{ mb: 1.5 }}>
                      <img src={item.image || '/placeholder.jpg'} alt={item.name} style={{ width: 48, height: 48, objectFit: "cover", borderRadius: 8, border: `2px solid ${COLORS.primary}20` }} />
                      <Box sx={{ flex: 1 }}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Box>
                        ...
                        <Stack direction="row" spacing={1} alignItems="center">
                          <IconButton size="small" disabled={(item.quantity || 1) <= 1} onClick={() => dispatch(incrementQuantityBySize({ id: item.id, size: item.size, delta: -1 }))}>
                            <Remove fontSize="small" />
                          </IconButton>
                          <Typography variant="body2" sx={{ color: COLORS.primary, fontWeight: 600, minWidth: 20, textAlign: 'center' }}>{item.quantity || 1}</Typography>
                          <IconButton size="small" onClick={() => dispatch(incrementQuantityBySize({ id: item.id, size: item.size, delta: 1 }))}>
                            <Add fontSize="small" />
                          </IconButton>
                        </Stack>
                      </Box>
                      <Box sx={{ textAlign: 'right' }}>
                        ...
                        <Typography variant="subtitle2" fontWeight={700} sx={{ color: COLORS.primary }}>{formatCurrency(item.price || 0)}</Typography>
                      </Box>
                    </Box>
                  ...
                  <Box sx={{ display: { xs: 'none', sm: 'none', md: 'contents' } }}>
                    <Stack direction="row" alignItems="center" spacing={1.5}>
                      <img src={item.image || '/placeholder.jpg'} alt={item.name} style={{ width: 56, height: 56, objectFit: "cover", borderRadius: 8, border: `2px solid ${COLORS.primary}20` }} />
                      <Box>
                        <Typography variant="subtitle2" sx={{ lineHeight: 1.3, color: COLORS.primary, fontWeight: 600 }}>{item.name}</Typography>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
                      <Stack direction="row" spacing={1} alignItems="center">
                        ...
                        <Typography variant="body2" sx={{ color: COLORS.primary, fontWeight: 600, minWidth: 24, textAlign: 'center' }}>{item.quantity || 1}</Typography>
                        <IconButton size="small" onClick={() => dispatch(incrementQuantityBySize({ id: item.id, size: item.size, delta: 1 }))}>
                          <Add fontSize="small" />
                        </IconButton>
                      </Stack>
                    ...
                    <Typography variant="subtitle2" textAlign="right" fontWeight={700} sx={{ color: COLORS.primary }}>{formatCurrency(item.price || 0)}</Typography>
                    <Box sx={{ textAlign: 'right' }}>
                      <IconButton color="error" aria-label="Xóa khỏi giỏ" onClick={() => dispatch(removeItem({ id: item.id, size: item.size }))}>
                        <DeleteOutline />
                      </IconButton>
            ...
            {/* Totals */}
            <Box sx={{ mb: 4, p: { xs: 1.5, sm: 2 }, backgroundColor: 'rgba(255,255,255,0.9)', borderRadius: 2, border: `1px solid ${COLORS.primary}20` }}>
              <Stack spacing={1}>
                <Row label="Tạm tính" value={formatCurrency(subtotal)} bold />
                <Row label="Phí vận chuyển" value={formatCurrency(shippingFee)} bold />
                {walletDeduction > 0 && (
                  <Row label="Thanh toán qua ví" value={`- ${formatCurrency(walletDeduction)}`} positive />
                )}
            ...
            {/* Payment method */}
            <Box sx={{ mt: 3 }}>
              <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", sm: "1fr 140px 120px 160px" }, gap: 2, px: { xs: 1, sm: 2 }, py: 1.5, backgroundColor: COLORS.primary, borderRadius: 2, color: 'white', fontWeight: 600 }}>
                <Stack direction="row" alignItems="center" spacing={1}>
              ...
              <Button
                ...
                fullWidth
                size="large"
                sx={{ mt: 2, textTransform: 'none', fontWeight: 700, color: 'white', ...DASHBOARD_STYLES.primaryButton, fontSize: { xs: '0.9rem', sm: '1rem' }, py: { xs: 1.5, sm: 2 }, '&.Mui-disabled': { background: '#e0e0e0', color: '#9e9e9e', boxShadow: 'none' } }}
              ...
              {!hasItems && (
                <Typography variant="caption" sx={{ mt: 1, display: 'block', textAlign: 'center', color: '#9e9e9e' }}>
      ...
      <ShippingAddressDialog open={showDialog} onClose={() => setShowDialog(false)} onSelect={(addr) => setSelectedAddress(addr)} />
    </>
  );
}
...
function Row({ label, value, bold, xbold, small, positive }) {
  return (
    <Box sx={{ display: "flex", justifyContent: "space-between", py: 0.5 }}>
      <Typography sx={{ color: positive ? 'success.main' : (small ? 'text.secondary' : COLORS.primary), fontWeight: xbold ? 900 : bold ? 600 : small ? 500 : 600, fontSize: small ? { xs: '0.75rem', sm: '0.875rem' } : { xs: '0.875rem', sm: '1rem' } }}>{label}</Typography>
      <Typography sx={{ color: positive ? 'success.main' : (small ? 'text.secondary' : COLORS.primary), fontWeight: xbold ? 900 : bold ? 600 : small ? 600 : 600, fontSize: small ? { xs: '0.75rem', sm: '0.875rem' } : { xs: '0.875rem', sm: '1rem' } }}>{value}</Typography>
    </Box>
  );
}
