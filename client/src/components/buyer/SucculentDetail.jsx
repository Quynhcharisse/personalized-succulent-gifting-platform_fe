    ...
    <Box sx={{ p: { xs: 1, sm: 2, md: 3 }, width: '100%', minHeight: '100vh', bgcolor: '#f8f8f8' }}>
      <Box sx={{ maxWidth: 1400, mx: 'auto' }}>
        <Breadcrumbs aria-label="breadcrumb" sx={{ mb: 2 }}>
          <Link underline="hover" color="inherit" sx={{ cursor: 'pointer' }} onClick={() => navigate('/buyer/succulent')}>Danh sách</Link>
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 2, alignItems: 'flex-start' }}>
          ...
          <Box sx={{ flex: 3, minWidth: 0 }}>
            <Card sx={{ borderRadius: 3, boxShadow: 2 }}>
              <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' } }}>
                ...
                <Box sx={{ p: 2, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <Box component="img" src={mainImage} alt={succulent.name} sx={{ width: { xs: '100%', md: 420 }, height: 420, objectFit: 'cover', borderRadius: 2 }} />
                  <Stack direction="row" spacing={1} sx={{ mt: 1, overflowX: 'auto' }}>
                    {Array.from(new Set([succulent.image, '/hinhKeSenda.jpg', '/senda.png'])).map((img, idx) => (
                      <Box key={idx} component="img" src={img} alt={`thumb-${idx}`} onClick={() => setMainImage(img)} sx={{ width: 72, height: 56, objectFit: 'cover', borderRadius: 1, cursor: 'pointer', border: mainImage === img ? '2px solid #4caf50' : '1px solid #eee' }} />
                    ))}
                  </Stack>
                </Box>
                ...
                <CardContent sx={{ flex: 1, minWidth: 0 }}>
                  ...
                  {userDesign && userDesign.length > 0 && (
                    <Box sx={{ mb: 2, p: 2, border: '1px dashed #4caf50', borderRadius: 2, background: '#f6fff6' }}>
            ...
            <Paper sx={{ mt: 3, p: 2, borderRadius: 2, width: '100%' }}>
              ...
              <Divider sx={{ mb: 2 }} />
              <Stack spacing={2}>
                {reviews.length === 0 && <Typography color="text.secondary">Chưa có đánh giá nào.</Typography>}
                {reviews.map((r, idx) => (
          ...
          <Box sx={{ flex: 1, minWidth: 0, position: { md: 'sticky' }, top: { md: 80 } }}>
            ...
            <Paper sx={{ p:2, mb:2, borderRadius:2 }}>
              ...
              <Stack spacing={1}>
                {newest.map(item => (
                  <Card key={item.id} sx={{ display:'flex', alignItems:'center', boxShadow:0, border:'1px solid #eee', borderRadius:2 }}>
                    <CardMedia component="img" image={item.image} alt={item.name} sx={{ width:64, height:64, objectFit:'cover', borderRadius:1 }} />
                    <CardContent sx={{ p:1 }}>
                      <Typography variant="subtitle2" fontWeight={600}>{item.name}</Typography>
                      <Typography variant="caption" color="success.main" fontWeight={600}>{item.price.toLocaleString('vi-VN')} ₫</Typography>
                    </CardContent>
                  </Card>
                ))}
              </Stack>
                ...
                {similar.map(item => (
                  <Card key={item.id} sx={{ display:'flex', alignItems:'center', boxShadow:0, border:'1px solid #eee', borderRadius:2 }}>
                    ...
                    <CardContent sx={{ p:1 }}>
                      <Typography variant="subtitle2" fontWeight={600}>{item.name}</Typography>
                      <Typography variant="caption" color="success.main" fontWeight={600}>{item.price.toLocaleString('vi-VN')} ₫</Typography>
                      <Button size="small" variant="text" sx={{ p:0, minWidth:0, mt:0.5, textTransform:'none' }} onClick={()=>navigate(`/buyer/succulent/${item.id}`)}>Xem</Button>
                    </CardContent>
                  </Card>
                ))}
              </Stack>
